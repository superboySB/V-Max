...
class BraxWrapper(environment.Wrapper):
    ...
    def reset(self, state: datatypes.SimulatorState, rng: jax.Array | None = None) -> EnvTransition:
        ...
        info = {
            "steps": jnp.ones(state.shape, dtype=jnp.int32),  # NOTE: This might be wrong
...
class VmapWrapper(environment.Wrapper):
    """Vectorizes Brax env."""
...
class AutoResetWrapper(environment.Wrapper):
    ...
    def step(self, env_transition: EnvTransition, action: jax.Array) -> EnvTransition:
        ...
        scenario_id = self._increment_scenario_id(env_transition, self._total_scenarios)
        new_env_transition = jax.tree.map(
            where_done,
            self._pull_next_scenario(scenario_id),
            next_env_transition,
        )
        new_env_transition.info["scenario_id"] = scenario_id

        return next_env_transition.replace(
            state=new_env_transition.state,
            observation=new_env_transition.observation,
            info=new_env_transition.info,
        )
