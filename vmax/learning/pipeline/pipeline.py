...
if typing.TYPE_CHECKING:
    ...
    from vmax.learning.algorithms import replay_buffer
    from vmax.simulator.wrappers import action
...
def run_training_off_policy(
    ...
    unroll_fn: action.generate_unroll,
    scan_length: int,
...
def _reshape_metrics(rollout_metrics: dict, sgd_metrics: dict) -> dict:
    ...
    return {
        ...
        **{f"learning/{name}": value for name, value in sgd_metrics.items()},
    }
...
def run_evaluation(
    ...
    def run_step(carry, _t):
        ...
        rollout_metrics = {
            "rewards": env_state.info["rewards"],
    ...
    return {**{f"eval/{name}": value for name, value in eval_metrics.items()}}
...
def prefill_replay_buffer(
    ...
    def run_random_step(carry, _x):
        ...
        state, transition = action.random_step(state, env, step_key, action_shape)
